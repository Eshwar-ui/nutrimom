...
/**
 * The product page body, shared by the public listing route and the admin
 * review route so a moderator judges exactly what a buyer would see.
 *
 * Presentational only (no data fetching, no server-only APIs), so it renders
 * happily from either a server or a client component.
 */
export function ListingDetail({
  ...
  backHref?: string;
  backLabel?: string;
  /** Slot above the product for context, e.g. the admin moderation bar. */
  banner?: React.ReactNode;
  /** Slot at the end, for overlays the page owns (dialogs, sheets). */
  after?: React.ReactNode;
  /** Buy/wishlist controls make no sense while reviewing an unapproved item. */
  showActions?: boolean;
}) {
